...
def make_pairs():
	...
	for i in range(len(source)):
		...
		target_txt = target[i][3]
		target_txt = cleaned_text(target_txt)
		target_txt_embeddings = txt_embeddings(model,target_txt)
		...
		for j in range(3,3+4*source[i][2],4):
			...
			src_txt = cleaned_text(src_txt)
			src_txt_embeddings = txt_embeddings(model,src_txt)
