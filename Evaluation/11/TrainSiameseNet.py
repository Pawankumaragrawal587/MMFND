...
# LSTM based model
source_input = Input(shape=(MAX_SEQUENCE_LENGTH, ), dtype='int32')
...
lstm_source = Bidirectional(LSTM(NUM_LSTM_UNITS))
lstm_target = Bidirectional(LSTM(NUM_LSTM_UNITS))

txtfeatsB1 = lstm_source(source_embed)
txtfeatsB = Dense(1024,activation="relu")(txtfeatsB1)
...
model = Model(inputs=[imgA, imgB, target_input, source_input], outputs=outputs)
...
history = model.fit([imgTrain[:, 0], imgTrain[:, 1], X1Train, X2Train], labelTrain[:],
	validation_data=([imgTest[:, 0], imgTest[:, 1], X1Test, X2Test], labelTest[:]),
...
y_preds = model.predict([imgTest[:, 0], imgTest[:, 1], X1Test, X2Test])
